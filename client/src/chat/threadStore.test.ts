...
describe('applyEvent', () => {
  it('appends a new part to its thread', () => {
    const state = applyEvent({}, { type: 'chat.part', scope: 'local', thread: 'dock', part: text('p1', 'hi') })
    expect(state.dock?.parts).toEqual([text('p1', 'hi')])
  })

  it('replaces a part with the same id in place', () => {
    const before = applyEvent({}, { type: 'chat.part', scope: 'local', thread: 'dock', part: text('p1', 'hi') })
    const after = applyEvent(before, { type: 'chat.part', scope: 'local', thread: 'dock', part: text('p1', 'hi there') })
  ...
  it('sets idle only on the named thread', () => {
    ...
    const state = applyEvent(busy, { type: 'chat.idle', scope: 'local', thread: LINE })
    expect(state[LINE]?.idle).toBe(true)
    expect(state.dock?.idle).toBe(false)
  })

  it('records the turn provenance on its thread', () => {
    const turn: ServerEvent = { type: 'chat.turn', scope: 'local', thread: LINE, agent: 'plan', model: { providerID: 'p', modelID: 'm' }, variant: null }
  ...
  it('stores an error and returns the thread to idle', () => {
    ...
    const state = applyEvent(busy, { type: 'chat.error', scope: 'local', thread: 'dock', message: 'boom' })
    expect(state.dock).toMatchObject({ error: 'boom', idle: true })
  })

  it('adds a permission ask once per id', () => {
    const once = applyEvent({}, { type: 'permission.ask', scope: 'local', thread: LINE, permission: ask('perm1') })
    const twice = applyEvent(once, { type: 'permission.ask', scope: 'local', thread: LINE, permission: ask('perm1') })
    expect(twice[LINE]?.permissions).toEqual([ask('perm1')])
  })

  it('removes a permission ask when done', () => {
    const asked = applyEvent({}, { type: 'permission.ask', scope: 'local', thread: LINE, permission: ask('perm1') })
    const done = applyEvent(asked, { type: 'permission.done', scope: 'local', thread: LINE, permissionID: 'perm1' })
  ...
  it('never touches another thread', () => {
    ...
    let state = applyEvent(before, { type: 'chat.part', scope: 'local', thread: LINE, part: text('l1', 'line') })
    state = applyEvent(state, { type: 'permission.ask', scope: 'local', thread: LINE, permission: ask('pl') })
    state = applyEvent(state, { type: 'permission.done', scope: 'local', thread: LINE, permissionID: 'pd' })
    expect(state.dock).toBe(before.dock)
  })

  it('ignores event types it does not know', () => {
    const before: ThreadsState = { dock: EMPTY_THREAD }
    const unknown = { type: 'chat.future', scope: 'local', thread: 'dock' } as unknown as ServerEvent
...
describe('applyHistory', () => {
  it('keeps streamed parts and merges history by id', () => {
    const streamed = applyEvent({}, { type: 'chat.part', scope: 'local', thread: 'dock', part: text('p2', 'live') })
