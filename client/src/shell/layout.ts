...
/** Derives the responsive layout from the viewport, the dock preference and the stored sidebar width. */
export function computeLayout(viewportW: number, dockPreference: boolean, sidebarW: number): Layout {
  const narrow = viewportW < NARROW_BELOW
  const dockOpen = dockPreference && viewportW >= DOCK_AUTO_COLLAPSE_BELOW
  const dockW = dockOpen ? DOCK_OPEN_WIDTH : DOCK_RAIL_WIDTH
